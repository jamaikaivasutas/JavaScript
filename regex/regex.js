...
let number2or3 = numberLength2Or3(text)
console.log(number2or3)

let evenStart = startsWithEven(text)
console.log(evenStart)

let numberStart = startsWithNumber(text)
console.log(numberStart)

let capital = capitalized(text)
console.log(capital)

function numberLength2Or3(text) {
    regex = /\b\d{2,3}\b/g
result = text.match(regex)
console.log(`\nKétjegyű és Háromjegyű számok keresése: ${regex}`)
return result
}
...
function startsWithEven(text) {
    regex = /[02468]\d*\b/g
    result = text.match(regex)
    console.log(`\n Páros számok keresése: ${regex}`)
    return result
}

function startsWithNumber(text) {
    regex = /\d\w*[a-zA-Z]\b\b/g
    result = text.match(regex)
    console.log(`\nSzámmal kezdődőek keresése: ${regex}`)
    return result
}
